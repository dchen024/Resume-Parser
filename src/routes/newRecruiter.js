    ...
    const handleNewUser = async () =>{
        ...
        if (isValid){
            try{
                ...
                const response = await axios.post(`http://localhost:8000/createRecruiter?email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}&fullname=${encodeURIComponent(fullName)}`);
    ...
    return(
        ...
        <div className="container">
            <div className={`NewUser ${hasErrors ? 'error' : ''} border border-primary rounded p-3`} style={{ '--height-increase': `${heightIncrease}px` }}>
                <h2 className="d-flex justify-content-center">Create Recruiter Account:</h2>
                <div className = "signIn-section d-flex justify-content-center">
                <Link to="/recruiterAuth" className="newUser" style={{textDecoration:'none'}}>
                ...
                <div className="newUser-section">
                    <label htmlFor='Email' className='Email'>
                        ...
                        {emailExists && <div className="error-message">This email already exists, {' '}  
                            <Link to="/recruiterAuth" className="emailExists" style={{textDecoration:'none'}}>
                                login!
                            </Link>
                        </div>}
